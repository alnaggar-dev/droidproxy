...
final class GrokAuthTests: XCTestCase {
    ...
    // MARK: - ServiceType recognition matches the written credential type

    func testServiceTypeRecognizesExactlyTheGrokCredentialFileType() {
        // The account list (AuthManager.parseAccount) and the proxy's loader
        // (GrokAuth.loadActiveCredentials) must agree on what counts as a Grok
        // account: recognize exactly the `type` GrokAuth writes, so the UI never
        // shows an account the proxy would then refuse to use.
        let writtenType = GrokAuth.credentialsJSON(
            GrokAuth.Credentials(access: "A", refresh: "R", expiresAtMs: 0, email: nil)
        )["type"] as? String
        XCTAssertEqual(writtenType, GrokAuth.authFileType)
        XCTAssertEqual(ServiceType(authFileType: GrokAuth.authFileType), .grok)
        XCTAssertEqual(ServiceType(authFileType: "GROK-CLI"), .grok)  // init lowercases
        // The non-canonical "grok" tag must NOT be recognized — GrokAuth never
        // writes it and the loader rejects it (regression guard for the dropped alias).
        XCTAssertNil(ServiceType(authFileType: "grok"))
    }
